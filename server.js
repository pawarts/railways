...
function req_result (id, res, req, file_name) {
    ...
    let query = `SELECT ${`users.login`}, ${`groups.group_title`}, ${`work.work_title`}, ${`work.id`}, ${`work.work_time`}, ${`subwork.subwork_title`}, ${`subwork.work_id`}, ${`subwork.subwork_status`}, ${`work.group_id`} FROM ${`just_do_it.groups`} LEFT JOIN ${`just_do_it.work`} ON ${`groups.id`} = ${`work.group_id`} INNER JOIN ${`just_do_it.users`} ON ${`users.id`} = ${`groups.user_id`} LEFT JOIN ${`just_do_it.subwork`} ON ${`work.id`} = ${`subwork.work_id`} WHERE ${`groups.user_id`} = ${`users.id`} AND ${`users.login`} = 'DF' ORDER BY \`work\`.\`work_time\``;
